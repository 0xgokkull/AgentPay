...
async function handlePaymentAgent(agent: AgentPaymentAgent) {
  ...
  console.log('  "Execute a 1000 token payment to 0x9876543210987654321098765432109876543210"');
  console.log('  "Pause the SplitPayRouter contract for maintenance"');
  console.log('  "Set treasury address to 0x1111111111111111111111111111111111111111"\n');
  ...
  try {
    ...
    if (result.executedActions.length > 0) {
      ...
      result.executedActions.forEach((action, i) => {
        console.log(`  ${i + 1}. ${action.contract}.${action.function}()`);
...
async function handleRegistrationAgent(agent: AgentRegistrationAgent) {
  ...
  const address = await prompt("Enter agent address to register (0x...): ");
...
async function handleTreasuryAgent(agent: TreasuryManagementAgent) {
  ...
  console.log("Examples:");
  console.log('  "Deposit 500 tokens into the vault for 0x1111111111111111111111111111111111111111"');
  console.log('  "Check total assets in the vault"');
  console.log('  "Withdraw 100 shares from vault for 0x2222222222222222222222222222222222222222"\n');
...
function showHelp() {
  ...
  console.log("  • Execute payments: 'Pay 100 tokens to 0x...'");
  console.log("  • Configure treasury: 'Set treasury to 0x...'");
  console.log("  • Manage pause state: 'Pause the router' / 'Unpause the router'");
  console.log("\nRegistration Agent Commands:");
  console.log("  • Register agent: Enter agent address when prompted");
  console.log("\nTreasury Agent Commands:");
  console.log("  • Deposit funds: 'Deposit [amount] into vault for 0x...'");
  console.log("  • Check balance: 'Check total assets'");
  console.log("  • Withdraw funds: 'Withdraw [amount] from vault'");
  console.log("\nSpecial Commands:");
  console.log("  • 'back' - Return to main menu from any agent");
